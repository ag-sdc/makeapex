...
# makeapex uses quite a few external programs during its execution. You
# need to have at least the following installed for makeapex to function:
...
MAKEAPEX_LINT_APEXBUILD=${MAKEAPEX_LINT_APEXBUILD:-1}
...
# Import libmakeapex
for lib in "$MAKEAPEX_LIBRARY"/*.sh; do
	source "$lib"
done
...
# Re-sources the APEXBUILD afterwards to allow for other variables that use $pkgver
update_pkgver() {
...
# Print 'source not found' error message and exit makeapex
missing_source_file() {
...
write_pkginfo() {
	...
	printf "# Generated by makeapex %s\n" "$makeapex_version"
	printf "# using %s\n" "$(fakeroot -v)"
...
EOF
	...
	# 3. Payload creation
	msg2 "$(gettext "Creating filesystem payload...")"
	...
	msg "$(gettext "Finished creating APEX: %s")" "$apex_file"
}
...
create_srcpackage() {
	...
	for file in "${all_sources[@]}"; do
		if [[ "$file" = "$(get_filename "$file")" ]] || (( SOURCEONLY == 2 )); then
			...
			if [[ ! -d "$absfile" ]]; then
				...
				cp -as "$absfile" "$srclinks/$pkgbase"
			fi
		fi
	done
	...
	shopt -s -o pipefail
	list_package_files | LANG=C bsdtar --no-fflags  -cnLf - --null --files-from - |
...
unset pkgname "${apexbuild_schema_strings[@]}" "${apexbuild_schema_arrays[@]}"
...
# test for available APEXBUILD functions
