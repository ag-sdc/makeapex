...
[[ -n "$LIBMAKEAPEX_LINT_APEXBUILD_XDATA_SH" ]] && return
LIBMAKEAPEX_LINT_APEXBUILD_XDATA_SH=1
...
lint_apexbuild_functions+=('lint_xdata')


lint_xdata() {
	local xdata_list entry key value ret=0

	get_apexbuild_all_split_attributes xdata xdata_list
	...
	for entry in "${xdata_list[@]}"; do
		...
		if [[ "${entry}" != "${key}=${value}" ]]; then
			...
			# The key "pkgtype" is reserved for makeapex.
			error "$(gettext "%s array: The key 'pkgtype' is reserved for makeapex.")" "xdata"
			ret=1
		fi
	done
