...
#	sccache - Cache compilations and reuse them to save time on repetitions
...
[[ -n "$LIBMAKEAPEX_BUILDENV_COMPILER_SH" ]] && return
LIBMAKEAPEX_BUILDENV_COMPILER_SH=1

MAKEAPEX_LIBRARY=${MAKEAPEX_LIBRARY:-"$(dirname "$0")/libmakeapex"}

source "$MAKEAPEX_LIBRARY/util/option.sh"

build_options+=('ccache' 'distcc' 'sccache')
buildenv_functions+=('buildenv_ccache' 'buildenv_distcc' 'buildenv_sccache')
...
buildenv_sccache() {
	if check_buildoption "sccache" "y"; then
		if check_buildoption "ccache" "y"; then
			error "$(gettext "%s and %s cannot be used together.")" "sccache" "ccache"
			return 1
		elif check_buildoption "distcc" "y"; then
			error "$(gettext "%s and %s cannot be used together.")" "sccache" "distcc"
			return 1
		elif [ -d /usr/lib/sccache/bin ]; then
