...
[[ -n "$LIBMAKEAPEX_SOURCE_GIT_SH" ]] && return
LIBMAKEAPEX_SOURCE_GIT_SH=1
...
export GIT_CONFIG_SYSTEM=${MAKEAPEX_GIT_CONFIG:-'/etc/makeapex.d/gitconfig'}
...
extract_git() {
	...
	if [[ $ref != "origin/HEAD" ]] || (( updating )) ; then
		if ! git checkout --force --no-track -B makeapex "$ref" --; then
			error "$(gettext "Failure while creating working copy of %s %s repo")" "${repo}" "git"
			plainerr "$(gettext "Aborting...")"
			exit 1
		fi
