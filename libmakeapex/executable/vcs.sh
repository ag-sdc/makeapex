...
[[ -n "$LIBMAKEAPEX_EXECUTABLE_VCS_SH" ]] && return
LIBMAKEAPEX_EXECUTABLE_VCS_SH=1
...
executable_vcs() {
	...
	for attr in depends makedepends; do
		get_apexbuild_attribute "$pkg" "$attr" 1 'deps'
		all_deps+=("${deps[@]}")

		get_apexbuild_attribute "$pkg" "${attr}_$CARCH" 1 'deps'
		all_deps+=("${deps[@]}")
	done
