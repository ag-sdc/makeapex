...
[[ -n "$LIBMAKEAPEX_SRCINFO_SH" ]] && return
LIBMAKEAPEX_SRCINFO_SH=1
...
apexbuild_extract_to_srcinfo() {
	...
	if get_apexbuild_attribute "$pkgname" "$attrname" "$isarray" 'outvalue'; then
...
srcinfo_write_section_details() {
	...
	for attr in "${singlevalued[@]}"; do
		apexbuild_extract_to_srcinfo "$1" "$attr" 0
	done

	for attr in "${multivalued[@]}"; do
		apexbuild_extract_to_srcinfo "$1" "$attr" 1
	done

	get_apexbuild_attribute "$1" 'arch' 1 'package_arch'
	for a in "${package_arch[@]}"; do
		# 'any' is special. there's no support for, e.g. depends_any.
		[[ $a = any ]] && continue

		for attr in "${multivalued_arch_attrs[@]}"; do
			apexbuild_extract_to_srcinfo "$1" "${attr}_$a" 1
		done
	done
}
